...
final class SimpleReporter extends A4Reporter {
    ...
    static String performEnumeration(PrintStream out, String filename) throws Exception {
        ...
        if (!sol.isIncremental())
            ...
            "Currently only MiniSatJNI and SAT4J are supported.";
        sol=sol.next();
        if (!sol.satisfiable())
    ...
    static void performRegularCommand(
        ...
        if (bundleIndex==-2) {
            ...
            TranslateAlloyToMetamodel.make(world, options.originalFilename, outf);
            rep.log(RESTORE2);
            ...
            if (ai==null) {
                ...
                result.add(tempXML);
                synchronized(SimpleReporter.class) { latestKodkod=ai; latestKodkodXML=tempXML; }
            }
        }
