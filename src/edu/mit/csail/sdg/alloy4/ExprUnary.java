...
public final class ExprUnary extends Expr {
     ...
     * <br><br>
     * "some X", "one X", and "lone X"
     * can be either formulas, or multiplicity constraints.
     * <br><br>
     * So we let the parser generate the former by default.
     * And whenever we construct a "<b>VarDecl(a,b)</b>" object,
     * or a "<b>ExprBinary.Op.IN(a,b)</b>" object, we call this method
     * on <b>b</b> to convert it into a multiplicity constraint.
     * <br><br>
     * This is safe, because in both cases, a formula would be illegal.
     * So the first form is always wrong.
     * <br><br>
     * And this is sufficient, because those are the only 2 places
    ...
    private ExprUnary(Pos p, Op o, Expr s, Type t) {
        super(p, t,
                (o==Op.SOMEMULT || o==Op.LONEMULT || o==Op.ONEMULT || o==Op.SETMULT)?1:0);
        op=o;
        sub=nonnull(s);
        ...
        if (s.mult>0)
            throw s.syntaxError("Multiplicity expression not allowed here");
    }
    ...
    public enum Op {
        ...
        /**
         * Constructs an untypechecked ExprUnary expression
         * with <b>this</b> as the operator.
         * @param p - the original position in the file
         * @param s - the subexpression
         */
        public final Expr make(Pos p, Expr s) {
            if (this==SETMULT && s.mult==2) return s;
            return new ExprUnary(p,this,s,null);
        }

        /**
         * Constructs a typechecked ExprUnary expression
         * with <b>this</b> as the operator, and <b>type</b> as the type.
         * @param p - the original position in the file
         * @param s - the subexpression
         * @param t - the type
         */
        public final Expr make(Pos p, Expr s, Type t) {
            if (this==SETMULT && s.mult==2) return s;
            return new ExprUnary(p,this,s,t);
        }
    }
}
