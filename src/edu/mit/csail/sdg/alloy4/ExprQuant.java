...
public final class ExprQuant extends Expr {
     ...
     * (as returned by size() and get()), since
    ...
    private ExprQuant(Pos p, Op o, List<VarDecl> l, Expr s, Type t) {
        ...
        if (o==Op.NO)
            throw internalError("The NO operator should have been desugared!");
        if (o==Op.ONE)
            throw internalError("The ONE operator should have been desugared!");
    ...
    public enum Op {
        ...
        /**
         * Constructs an ExprQuant expression with <b>this</b> as the operator.
         * @param p - the original position in the file
         * @param l - the list of variable declarations
        ...
        public final Expr make(Pos p, List<VarDecl> l, Expr s, Type t) {
            ...
            //
            Type tf=Type.FORMULA;
            ArrayList<VarDecl> l2=new ArrayList<VarDecl>();
            if (l==null) throw new ErrorInternal(p,null,"NullPointerException");
            l=new ArrayList<VarDecl>(l);
            for(int i=0; i<l.size(); i++)
                if (l.get(i)==null)
                    throw new ErrorInternal(p,null,"NullPointerException");
            if (this==NO) {
                if (l.size()==1)
                    return new ExprQuant(p,ALL,l,ExprUnary.Op.NOT.make(s.pos,s,tf),tf);
                l2.add(l.get(0));
                l.remove(0);
                return new ExprQuant(p,ALL,l2,NO.make(p,l,s,tf),tf);
            }
            if (this==ONE) {
                Expr a=LONE.make(p,l,s,tf), b=SOME.make(p,l,s,tf);
                return ExprBinary.Op.AND.make(p,a,b,tf);
            }
            if (this==ALL || this==SOME || this==SUM) if (l.size()>1) {
                l2.add(l.get(0));
                l.remove(0);
                return new ExprQuant(p,this,l2,this.make(p,l,s,tf),tf);
            }
            if (this==LONE && l.size()>1) {
                l2.add(l.get(0));
                l.remove(0);
                Expr a=new ExprQuant(p,LONE,l2,SOME.make(p,l,s,tf),tf);
                Expr b=new ExprQuant(p,ALL,l2,LONE.make(p,l,s,tf),tf);
                return ExprBinary.Op.AND.make(p,a,b,tf);
            }
