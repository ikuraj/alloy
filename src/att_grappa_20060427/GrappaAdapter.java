...
implements GrappaConstants, GrappaListener, ActionListener
{
	...
	public void grappaPressed(Subgraph subg, Element elem, GrappaPoint pt, int modifiers, GrappaPanel panel) {
		...
		// Note by Alloy developers:
		// The following paragraph of code is a deliberate alteration
		// of the Grappa code, in order to let Ctrl-Left-Click bring up the popup menu, just like Right-Click.
		// (Since on most Mac computers, the mouse only has 1 button)
		int button1=InputEvent.BUTTON1_MASK;
		int button2=InputEvent.BUTTON2_MASK;
		int button3=InputEvent.BUTTON3_MASK;
		int button23=button2|button3;
		if ((modifiers&button23)!=0 && (modifiers&button23)==modifiers) { }
		else if ((modifiers&button1)!=0 && (modifiers&InputEvent.CTRL_MASK)!=0
				&& (modifiers&(button1|InputEvent.CTRL_MASK))==modifiers) {}
		else if ((modifiers&button1)!=0 && (modifiers&InputEvent.CTRL_DOWN_MASK)!=0
				&& (modifiers&(button1|InputEvent.CTRL_DOWN_MASK))==modifiers) {}
		else return;
