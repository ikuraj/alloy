	...
	Cloneable, ImageObserver, Observer, Shape
{
	...
	case NODE:
		...
		try {
			...
			shape = (Shape)(ccustom.newInstance(new Object[] { node,  new Double(pos.x - (width/2.0)), new Double(pos.y - (height/2.0)), new Double(width), new Double(height) }));
		    } else {
			shape = (Shape)(ccustom.newInstance(new Object[] { node, new Double(pos.x), new Double(pos.y), new Double(width), new Double(height) }));
		    }
		}
		catch(Exception e) {
