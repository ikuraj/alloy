...
sig State {
  waiting, active, contending, elected: set Node,
  parentLinks: set Link,
  queue: Link -> one Queue,
  op: Op -- the operation that produced the state
} {
  no (waiting & active)
  no ((waiting + active) & contending)
  no ((waiting + active + contending) & elected)
  ...
  s'.op = ReadReqOrAck => {
    ...
    some x: Link {
      x.target in s.(active + contending)
       & (PeekQueue [s, x, Ack] => s'.contending else s'.active)
      NoChangeExceptAt [s, s', x.target]
      ! IsEmptyQueue [s, x]
      ReadQueue [s', s, x]
      }}
...
run Execution for 1 Int, 7 Op, 2 Msg,
...
check NoOverflow for 1 Int, 7 Op, 2 Msg,
...
/* defined variables:
 *
 * notOp = {s: State, o: Op | !(o in s.op)}
 * after = {pre, post: State | next(pre) = post}
 */
