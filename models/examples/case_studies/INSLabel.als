...
}
{
  ...
  all n: nodes | all c,c': n.children | /* no(c&c') => */ c.label!=c'.label
...
pred Query::Conforms(a:Advertisement) {
  ConformsAux[this, a, this.root, a.root]
}

fact LookupFixPoint {
  all db: DB, q: Query, T: Node, n: Node, r: Record |
    r in LookupAux[db,q,T,n] <=>                                  // record r is in the result if and only if
    {
         ...
         nv.label = Wildcard =>                                    //  wildcard matching
           ...
           (some Tv: Ta.(db.children) {                             //  normal matching                                     
             ...
             no nv.(q.children) =>                                 //   if Tv is a leaf-node
               ...
               r in LookupAux[db,q,Tv,nv] }) }                     //   else r is a record of the recursive call at Tv
    }
}
